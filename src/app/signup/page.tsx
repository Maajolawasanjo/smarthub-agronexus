...
export default function SignUpPage() {
  ...
  function validateBuyer(data: BuyerForm): Partial<BuyerForm> {
    ...
    else if (!PW_RULES.every(r => r.test(data.password))) e.password = "Password doesn't meet all security requirements";
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    if (tab === "buyer") {
      setBuyerTouched({ fullName: true, email: true, phone: true, password: true, confirmPassword: true });
      ...
      try {
        ...
        if (!res.ok) {
          const errMsg = data.error || "Failed to create buyer account.";
        ...
        const errMsg = "Unable to connect to authentication server.";
        setServerError(errMsg);
        toast(errMsg, "error");
      } finally {
        setIsLoading(false);
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          const errMsg = data.error || "Failed to create farmer account.";
          setServerError(errMsg);
          toast(errMsg, "error");
        } else {
          setUserFromAuth(data.user);
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex flex-col items-center justify-center p-4 font-sans">
      <motion.div
        ...
        className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-gray-100 p-6 md:p-10 relative"
      >
        {/* Back */}
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          <AnimatePresence mode="wait">
            {tab === "buyer" ? (
              ...
              >
                ...
                <Field label="Phone No" error={bTch.phone ? bErr.phone : undefined}>
                  <div className="relative">
                    <input
                      type="tel"
                      placeholder="08012345678"
                      value={buyer.phone}
                      onChange={e => { setBuyer(p => ({ ...p, phone: e.target.value })); if (bTch.phone) setBuyerErrors(validateBuyer({ ...buyer, phone: e.target.value })); }}
                      onBlur={() => touchBuyer("phone")}
                      className={cn(inputCls(!!(bTch.phone && bErr.phone), !!buyer.phone), "pr-10")}
                      autoComplete="tel"
                    />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field label="State" error={fTch.state ? fErr.state : undefined}>
                    <select
                      ...
                      className={cn(
                        "w-full px-4 py-3 rounded-full border-2 text-sm outline-none transition-all duration-200 bg-white appearance-none cursor-pointer",
          ...
          >
            <div className="relative">
              ...
              />
              <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 cursor-pointer">
                {showPw ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            </div>

            {/* Password strength checklist with spring scale pop */}
            {currentPw && (
                ...
                {PW_RULES.map(rule => {
                  ...
                  return (
                    <div key={rule.label} className="flex items-center gap-2">
                      <motion.div
                        animate={{
                          scale: passed ? [0.8, 1.25, 1] : 1,
                          backgroundColor: passed ? "#1B4D28" : "#E5E7EB",
                        }}
                        transition={{ duration: 0.25 }}
                      ...
                      <span className={cn("text-[11px] transition-colors duration-200", passed ? "text-[#1B4D28] font-medium" : "text-gray-400")}>
                        {rule.label}
                      </span>
                    </div>
                  );
          ...
          >
            <div className="relative">
              <input
                ...
                autoComplete="new-password"
              />
              <button type="button" onClick={() => setShowConfirmPw(!showConfirmPw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 cursor-pointer">
        ...
        <p className="text-center text-xs text-gray-500 mt-6">
          ...
          <Link href="/login" className="font-bold text-[#1B4D28] hover:underline">Sign In</Link>
        </p>
      </motion.div>
    </div>
  );
