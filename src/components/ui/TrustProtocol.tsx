...
import { MotionFade, MotionStagger, MotionStaggerItem, AnimatedCounter } from "@/components/motion";
...
const metrics = [
    { value: 99.2, decimals: 1, suffix: "%", label: "Average Commodity Purity", highlight: "Export Grade" },
    { value: 100, decimals: 0, suffix: "%", label: "Escrow-Protected Capital", highlight: "Zero Advance Risk" },
    { value: 14, decimals: 0, suffix: " Days", label: "Average Farm-to-Port Lead Time", highlight: "Rapid Logistics" },
    { value: 50, decimals: 0, suffix: "+", label: "Vetted Farming Cooperatives", highlight: "Direct Off-Take" },
];

export function TrustProtocol() {
    return (
        <section className="w-full py-28 md:py-36 bg-gradient-to-b from-[#EEF2EE] via-white to-[#EEF2EE] font-sans overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 md:px-8">
                ...
                {/* 4-Step Cards Grid */}
                <MotionStagger staggerDelay={0.12} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 mb-16">
                    {steps.map((step) => {
                        const Icon = step.icon;
                        return (
                            <MotionStaggerItem key={step.number} direction="up" distance={30} className="h-full">
                                <motion.div
                                    whileHover={{ y: -8 }}
                                    transition={{ type: "spring", stiffness: 350, damping: 20 }}
                                    className="relative bg-white rounded-3xl p-7 border border-gray-200/70 shadow-sm hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between group h-full cursor-pointer"
                                >
                                    <div>
                                        {/* Card Header: Step number & Icon */}
                                        <div className="flex items-center justify-between mb-6">
                                            <span className="text-3xl font-extrabold text-[#1B4D28]/20 group-hover:text-[#4CAF50] transition-colors duration-300">
                                                {step.number}
                                            </span>
                                            <div className="w-12 h-12 rounded-2xl bg-[#EEF2EE] text-[#1B4D28] flex items-center justify-center group-hover:bg-[#1B4D28] group-hover:text-white group-hover:rotate-6 transition-all duration-300 shadow-sm">
                                                <Icon size={22} />
                                            </div>
                                        </div>
                ...
                <MotionFade direction="up" distance={30} delay={0.2}>
                    <div className="bg-[#0a1f0f] rounded-3xl p-8 md:p-12 text-white shadow-2xl relative overflow-hidden">
                        <motion.div
                            ...
                            className="absolute top-0 right-0 w-96 h-96 bg-[#4CAF50]/15 rounded-full blur-3xl pointer-events-none"
                        />
                        ...
                        <div className="relative z-10 mt-10 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
                            ...
                            <Link
                                href="/how-it-works"
                                className="inline-flex items-center gap-2 text-xs md:text-sm font-bold text-[#e8b84a] hover:text-white transition-colors group"
                            >
