...
for r in regions:
    ...
    cmd = f"DATABASE_URL='{url}' npx tsx scratch/test_registration.ts 2>&1"
    res = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=10)
    out = res.stdout + res.stderr
    if "Existing user result" in out:
        print(f"=================================================")
        print(f"SUCCESS MATCHING REGION: {host}")
        print(f"=================================================")
        break
    else:
        err_lines = [l for l in out.splitlines() if "FATAL" in l or "Can't reach" in l or "tenant" in l or "Error" in l]
        print(f"Region {r}: {err_lines[0] if err_lines else 'FAIL'}")
